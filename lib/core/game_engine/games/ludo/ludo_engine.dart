...
class LudoToken {
  ...
  final int id; // 0-3
  int position; // -1=home, 0-55=track, 56=finished
...
class LudoState {
  final List<List<LudoToken>> tokens; // [color][0..3]
  final int currentPlayer; // 0=red(human), 1=green(bot), 2=blue(bot), 3=yellow(bot)
...
class LudoEngine {
  ...
  // Safe squares (star positions on main track 0-51)
  static const safes = {0, 8, 13, 21, 26, 34, 39, 47};
  // Starting positions for each color (position on main track)
  ...
  bool moveToken(int tokenId) {
    ...
    if (t.isHome) {
      ...
      t.position += dice;
      if (t.position >= 56) t.position = 56; // finished
    }
  ...
  int botRollAndMove() {
    ...
    if (_state.phase == LudoPhase.moving && _state.movableTokenIds.isNotEmpty) {
      // Bot prefers: finish > capture > advance furthest
      final player = _state.currentPlayer;
      ...
      for (final id in _state.movableTokenIds) {
        ...
        if (!t.isHome) {
          ...
          priority = 100; // Bring out from home
        }
