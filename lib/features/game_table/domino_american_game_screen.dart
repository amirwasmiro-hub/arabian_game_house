...
class DominoAmericanGameScreen extends StatefulWidget {
  ...
  @override State<DominoAmericanGameScreen> createState() => _DominoAmericanGameScreenState();
}

class _DominoAmericanGameScreenState extends State<DominoAmericanGameScreen> {
  final _engine = DominoEngine(mode: DominoMode.american);
  ...
  Widget _buildTopBar(DominoState s) => Container(
    ...
    child: Row(
      children: [
        ...
        SizedBox(width: 6.w),
        Text('دومينو أمريكاني 🀄', style: GoogleFonts.cairo(fontSize: 13.sp, color: const Color(0xFFFFD700), fontWeight: FontWeight.bold)),
        SizedBox(width: 12.w),
        ...
        Flexible(
          ...
          child: Container(
            ...
            child: Text(
              s.message,
              style: GoogleFonts.cairo(color: Colors.lightBlue, fontSize: 11.sp),
              maxLines: 1,
  ...
  Widget _buildChainArea(DominoState s) {
    ...
    final left = s.leftEnd ?? 0;
    final right = s.rightEnd ?? 0;
    final sum = left + right;
    final isMultipleOf5 = sum % 5 == 0 && s.chain.length > 1;
          ...
          child: Row(
            children: [
              ...
              Container(
                padding: EdgeInsets.symmetric(horizontal: 8.w, vertical: 2.h),
                decoration: BoxDecoration(
                  color: isMultipleOf5 ? Colors.amber.withValues(alpha: 0.25) : Colors.purple.withValues(alpha: 0.2),
                  borderRadius: BorderRadius.circular(8.r),
                  border: Border.all(color: isMultipleOf5 ? Colors.amber : Colors.purple.withValues(alpha: 0.5)),
                ),
                child: Text(
                  'المجموع: $sum ${isMultipleOf5 ? "✨ مضاعف 5 (+$sum)!" : ""}',
                  style: GoogleFonts.cairo(color: isMultipleOf5 ? Colors.amber : Colors.purpleAccent, fontSize: 10.sp, fontWeight: isMultipleOf5 ? FontWeight.bold : FontWeight.normal),
                ),
              ),
              const Spacer(),
  ...
  Widget _tile(DominoPiece p, {bool onTable = false, bool isValid = true, bool isSelected = false}) => Container(
    ...
    decoration: BoxDecoration(
      color: onTable ? Colors.black87 : isValid ? Colors.black87 : Colors.grey.shade900,
      borderRadius: BorderRadius.circular(4.r),
      border: Border.all(color: isSelected ? const Color(0xFFFFD700) : onTable ? const Color(0xFF1976D2) : isValid ? const Color(0xFFFFD700).withValues(alpha: 0.6) : Colors.white12, width: isSelected ? 2 : 1),
      boxShadow: isSelected ? [BoxShadow(color: const Color(0xFFFFD700).withValues(alpha: 0.5), blurRadius: 6)] : null,
    ),
    child: Column(
      mainAxisAlignment: MainAxisAlignment.spaceEvenly,
      children: [
        Text('${p.a}', style: GoogleFonts.montserrat(color: isValid ? Colors.white : Colors.white38, fontWeight: FontWeight.bold, fontSize: onTable ? 10.sp : 11.sp)),
        Divider(color: const Color(0xFFFFD700), height: 1),
        Text('${p.b}', style: GoogleFonts.montserrat(color: isValid ? Colors.white : Colors.white38, fontWeight: FontWeight.bold, fontSize: onTable ? 10.sp : 11.sp)),
      ],
    ),
  );
